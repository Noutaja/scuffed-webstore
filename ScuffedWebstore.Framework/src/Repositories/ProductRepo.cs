...
public class ProductRepo : BaseRepo<Product>, IProductRepo
{
    ...
    {
        return _data.AsNoTracking().Include(p => p.CategoryID).Include(p => p.Images).Where(p => p.Title.Contains(options.Query)).Skip(options.Offset).Take(options.Limit);
    }

    public override IEnumerable<Product> GetAll(GetAllParams options)
    {
        return _data.AsNoTracking().Include(p => p.CategoryID).Skip(options.Offset).Take(options.Limit);
    }
